...
const filteredMerchants = computed(() => {
  ...
  return store.merchants.filter(m => m.name?.toLowerCase().includes(q))
})
...
function reset() {
  Object.assign(draft, { id: '', type: 'company', name: '', addresses: [], contacts: [], notes: '' })
}
function save() {
  if (!draft.name.trim()) return
  if (!draft.id) draft.id = uid('mer_')
  const copy = JSON.parse(JSON.stringify(draft))
  const idx = store.merchants.findIndex(x => x.id === copy.id)
  if (idx === -1) store.merchants.push(copy)
  else store.merchants.splice(idx, 1, copy)
  reset()
}
function remove(id) {
  removeById(store.merchants, id)
...
<template>
  <div class="row g-3">
    ...
    <div class="col-lg-6">
      <SectionCard :title="draft.id ? 'Edit Merchant' : 'New Merchant'">
        <div class="row g-3">
          <div class="col-md-4">
            <label class="form-label">Type</label>
            <select class="form-select" v-model="draft.type">
          ...
          <div class="col-md-8">
            ...
            <input class="form-control" v-model="draft.name" placeholder="e.g., Acme Inc." />
          </div>
          <div class="col-12">

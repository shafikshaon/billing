...
function removeRow(t){
  if (confirm(`Delete payment term "${t.name}"?`)) {
    removeById(store.paymentTerms, t.id)
  }
}
function summary(t) {
  // Build a small schedule summary, e.g., "100% in 30d" or "50/50 in 0/30d"
  const parts = (t.schedule || []).map(l => {
    const v = l.valueType === 'percent' ? `${l.valueAmount}%` : `৳${Number(l.valueAmount).toFixed(0)}`
    const d = l.dateType === 'days_after' ? `${l.days||0}d` : `day ${l.dayOfMonth}`
    return `${v}@${d}`
  })
  return parts.join(', ')
}
